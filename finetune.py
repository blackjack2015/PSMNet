...
parser.add_argument('--datapath', default='/media/jiaren/ImageNet/data_scene_flow_2015/training/',
                    help='datapath')
parser.add_argument('--logFile', default='finetune.log',
                    help='log path')
...
def main():
	...
	start_full_time = time.time()
	   ...
	   logger.info('epoch 0 total 3-px error in val = %.3f' %(total_test_loss/len(TestImgLoader)*100))
	   if total_test_loss/len(TestImgLoader)*100 > max_acc:
		max_acc = total_test_loss/len(TestImgLoader)*100
	   logger.info('MAX epoch %d total test error = %.3f' %(max_epo, max_acc))
	...
	for epoch in range(1, args.epochs+1):
	   ...
	   if total_test_loss/len(TestImgLoader)*100 > max_acc:
		max_acc = total_test_loss/len(TestImgLoader)*100
		max_epo = epoch
	        savefilename = args.savemodel+'best_'+str(epoch)+'.tar'
	        torch.save({
	     	    'epoch': epoch,
